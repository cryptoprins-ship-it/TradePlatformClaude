...
def test_load_settings_reads_env_from_dotenv_file(tmp_path, monkeypatch):
    cfg = tmp_path / "config.yaml"
    cfg.write_text(textwrap.dedent("""
        symbol: "BTC/USDT:USDT"
        timeframe: "1h"
        ...
        risk:
          ...
          single_position: true
    """))
    (tmp_path / ".env").write_text("ANTHROPIC_API_KEY=from-dotenv-file\n")
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)

    s = load_settings(str(cfg))

    assert s.anthropic_api_key == "from-dotenv-file"
